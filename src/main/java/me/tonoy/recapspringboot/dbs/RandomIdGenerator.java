...
public class RandomIdGenerator implements IdentifierGenerator {
    ...
    private static final int DEFAULT_ID_LENGTH = 16;
    private int idLength;

    @Override
    public void configure(Type type, Properties parameters, ServiceRegistry serviceRegistry) {
        idPrefix = ConfigurationHelper.getString(ID_PREFIX_PARAMETER, parameters, DEFAULT_ID_PREFIX);
        idLength = ConfigurationHelper.getInt(ID_LENGTH_PARAMETER, parameters, DEFAULT_ID_LENGTH);
    }

    @Override
    public Object generate(SharedSessionContractImplementor session, Object object) {
        return RandomString.next(idPrefix, idLength);
    }
}
